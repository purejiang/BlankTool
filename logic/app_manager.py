...
from common.config import DEFAULT_CONFIG_PATH, AppConfig, Config, UserConfig
...
class AppManager():
    ...
    def __checkToolDir(cls, logger) -> bool:
        ...
        for tool_dir in app_dir:
            if not FileHelper.fileExist(tool_dir):
                logger.info("检测到不存在目录：{}，重建".format(tool_dir))
    ...
    def initUserConfig(cls, logger):
        ...
        """
        logger.info("初始化用户配置")
        user_config_file = UserConfig.getConfigFile()

        if not FileHelper.fileExist(user_config_file):
            logger.info("检测到不存在配置文件: {}，重置为默认配置".format(user_config_file))

            FileHelper.copyFile(DEFAULT_CONFIG_PATH, user_config_file)
        return True
    ...
    def initApplication(cls, progress_callback) -> bool:
        ...
        progress_callback(15, "初始化环境变量", "", init_result)
        if not init_result:
            return False
        check_re_result = cls.__checkRe(logger)

        progress_callback(30, "检查运行环境", "", check_re_result)
        if not check_re_result:
            return False
        load_result = cls.__loadRcc(logger)

        progress_callback(45, "加载 .rcc", "", load_result)
        ...
        progress_callback(60, "检查工具目录", "", check_dir_result)
        if not check_dir_result:
            return False
        
        update_result = cls.checkUpdate(logger)
        progress_callback(75, "检查更新", "", update_result)
        if not update_result:
            return False
        
        update_result = cls.checkUpdate(logger)
        progress_callback(90, "初始化用户配置", "", update_result)
        if not update_result:
            return False
        return True
